...
async def booking_phone(message: Message, state: FSMContext):
    ...
    who_keyboard = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text='Для себя')],[KeyboardButton(text='Для ребенка')]], resize_keyboard=True)
    await message.answer('Для кого тренировка?', reply_markup=who_keyboard)

@dp.message(BookingForm.who)
async def booking_who(message: Message, state: FSMContext):
    await state.update_data(who=message.text)
    if message.text == 'Для ребенка':
        ...
        day_keyboard = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text='Вторник')],[KeyboardButton(text='Четверг')],[KeyboardButton(text='Суббота')],[KeyboardButton(text='Воскресенье')]], resize_keyboard=True)
        await message.answer('Выберите день:', reply_markup=day_keyboard)

@dp.message(BookingForm.child_name)
async def booking_child_name(message: Message, state: FSMContext):
    await state.update_data(child_name=message.text)
    await state.set_state(BookingForm.child_age)
    await message.answer('Введите возраст ребенка:')

@dp.message(BookingForm.child_age)
async def booking_child_age(message: Message, state: FSMContext):
...
async def booking_day(message: Message, state: FSMContext):
    ...
    time_keyboard = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text='09:00')],[KeyboardButton(text='11:00')],[KeyboardButton(text='13:00')],[KeyboardButton(text='15:00')],[KeyboardButton(text='17:00')],[KeyboardButton(text='19:00')]], resize_keyboard=True)
...
async def booking_time(message: Message, state: FSMContext):
    ...
    await message.answer('Тренер свяжется с вами для подтверждения.')
